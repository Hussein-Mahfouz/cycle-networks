...
%DIF DEL paper-original-submission.tex   Sat Sep  3 20:07:29 2022
%DIF ADD README.tex                      Sat Sep  3 20:07:12 2022
...
infrastructure that is based on motivators and deterrents to cycling. \DIFaddbegin \DIFadd{In this paper we
...
\DIFdelbegin %DIFDELCMD < \hypertarget{network-level-approaches}{%
%DIFDELCMD < \subsection{Network-Level Approaches}\label{network-level-approaches}}
%DIFDELCMD < %%%
...
components}. \DIFdelbegin \DIFdel{(}\textbf{\DIFdel{natera2019data?}}%DIFAUXCMD
\DIFdel{) for example,
...
data to identify \DIFdelbegin \DIFdel{7 }\DIFdelend \DIFaddbegin \DIFadd{seven }\DIFaddend different communities in the city of Isfahan, Iran.
...
Our work builds on these \DIFdelbegin \emph{\DIFdel{community finding}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{community finding }\DIFaddend approaches by proposing a
similar greedy network expansion algorithm for cycle network expansion
\DIFdelbegin \DIFdel{within communities}\DIFdelend \DIFaddbegin \emph{\DIFadd{within communities}}\DIFaddend . We incorporate community finding methods for study
area partitioning with weighted routing to avoid links that are
stressful to cycle on. In doing so, we propose an approach that \DIFdelbegin \DIFdel{accounts }\DIFdelend \DIFaddbegin \DIFadd{ensures a better distribution
investments across the study area while also accounting }\DIFaddend for motivators and deterrents to cycling.
We propose three sub-methods that address some of the limitations of previous
studies. These limitations include (a) an inherent bias \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{when }\DIFaddend basing network
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{data/Manchester/Plots/histogram_distance_cycling_potential_current_all} 
%DIFDELCMD < %%%
...
\ref{fig:componentsandGCC}). Here\DIFaddbegin \DIFadd{, }\DIFaddend however, we find that there is little
difference between both \DIFdelbegin \DIFdel{Algorithms}\DIFdelend \DIFaddbegin \DIFadd{algorithms}\DIFaddend .
...
and low-stress networks. \DIFaddbegin \DIFadd{A core contribution of this approach is that it insures
...
cities\footnote{The results are easily reproducible for all UK cities, and can also be reproduced for cities elsewhere given the availability of commuter data. Instructions for reproducing the results are detailed in the \textbf{README} file in the R folder of this \href{https://github.com/Hussein-Mahfouz/cycle-networks}{Github repository}}\DIFaddbegin \DIFadd{.
}\DIFaddend 
...
encourages investment in cycling infrastructure \DIFdelbegin \DIFdel{to
increase the connectivity of existing cycling infrastructure
}\emph{\DIFdel{and}} %DIFAUXCMD
\DIFdel{investment }\DIFdelend that addresses geographical and
social inequalities. This \DIFaddbegin \DIFadd{is done by explicitly ensuring that infrastructure improvements
...
This }\DIFaddend ability to address inequalities in network prioritization is particularly
...
\leavevmode\vadjust pre{\DIFaddbegin \hypertarget{ref-manchestercyclingbee2020}{}}%DIF > 
\DIFadd{Greater Manchester, T. for, 2020. Change a region to change a nation - greater manchester's walking and cycling investment plan.
}
...
\leavevmode\vadjust \DIFadd{pre}{\hypertarget{ref-orozco2020data}{}}%DIF > 
\DIFadd{Orozco, L.G.N., Battiston, F., Iñiguez, G., Szell, M., 2020. Data-driven strategies for optimal bicycle network growth. Royal Society open science 7.
}\DIFaddend 
