...
#linenumbers: true
numbersections: true
abstract: "Understanding the motivators and deterrents to cycling is essential for creating infrastructure that is successful in getting more people to adopt cycling as a mode of transport. The aim of this paper is to develop methods to prioritise cycling infrastructure, accounting for cyclist preference and UK government policies to create Low Traffic Neighborhoods. Distance decay, routing, and network analysis techniques are used to examine where future cycling demand is most likely to arise, how such demand could be accomodated by the street network, and how to ensure a fair distribution of investment. The city of Manchester, which has substantial cycling ambitions, is used to demonstrate the methods which, we demonstrate, can be applied to any city. Flexible routing profiles that are adjustable to local needs can avoid high-stress road segments, enabling the identification of networks that balance directness against the need for safe and stress-free routes. Community detection is used to partition the city and evaluate results from 'utilitarian' and 'egalitarian' policy perspectives. We conclude that the 'egalitarian' model prioritisation approach demonstrated in this paper has great potential to support cost-effective strategic cycle network planning internationally. \\par\\textbf{Keywords:} cycling networks, low-traffic neighborhoods, routing, transport equity"
