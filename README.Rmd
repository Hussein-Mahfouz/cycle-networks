...
The methodology is designed so as to respect both the needs of the users and the ambitions of transport planners and local/regional level government stakeholders. It can can be used as a starting point for introducing a direct, connected, and low-stress network for any city by leveraging available open data. The only data necessary for reproducing it is the road network (from OSM), its topography (from satellite imagery), and commuter data (from the national census)^[The results are easily reproducible for all UK cities, and can also be reproduced for cities elsewhere given the availability of commuter data. Instructions for reproducing the results are detailed in this [Github repository](https://github.com/Hussein-Mahfouz/cycle-networks)]. It should be noted that the recommendations are only as good as the granularity of the available data.  
