...
We used the city of Manchester as a case study.
The city is situated in the region of Greater Manchester, which has ambitions to become the first UK region with a carbon neutral transport network.
These ambitions are supported by plans for a 1800 mile walking and cycling network (as part of the 'Bee Network') that will be delivered over 10 years [@manchestercyclingbee2020].
...
Manchester is a city in the North West of England, and is the most populous of the 10 metropolitan boroughs that make up Greater Manchester (\@ref(fig:manchestermap)).
...
-   The city of Manchester, which is used by visitors to refer to the city centre and surrounding areas. This definition is formalised in the Major Towns and Cities (TCITY) dataset provided by the Office for National Statistics [@ons2015cities].
...
As shown in Figure \@ref(fig:manchestermap) (right), the high density of the central area allows relatively high levels of cycling compared to the rest of the city.

```{r manchestermap, echo=FALSE, fig.cap="Maps of Greater Manchester (left) and the case study area - City of Manchester (right)", fig.show='hold', fig.align="center"}
...
In the context of other cities defined in the TCITY dataset, Manchester is a large city, with a mid-level of cycling (Figure \@ref(fig:manchestercyclinglevels)).
If the ambitions of the Bee Network are realized in the coming years, this mode share could go up significantly.

```{r manchestercyclinglevels, echo=FALSE, fig.cap="Cycling levels in cities across the country", out.width="85%", fig.show='hold', fig.align="center"}


knitr::include_graphics("./data/uk_cities_mode_share_2021.png")
```
...
The network prioritization is based strongly on potential cycling demand calculations.
While this provides a solid supplementary tool for network planning, it would be more useful as part of a multi-criteria approach that considers on-the ground information from existing cyclists, road geometries, and traffic analysis.
@grise2018if propose a multi-criteria approach that includes the first of these points.
