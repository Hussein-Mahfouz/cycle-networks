...
The 2015 Paris agreement [@agreement2015paris] acknowledged that
...
casualties and physical inactivity [@brand_climate_2020].
...
4th highest cause of death globally [@kohl2012pandemic]. Various
...
[@celis2017association; @jarrett2012effect;
@patterson2020associations]. In the wake of the Covid-19 pandemic, and
...
> vehicles and pedestrians - [@departmentgearchange2020]"
...
A considerable amount of research has been done to understand what gets people to cycle.
Segregated cycling infrastructure^[*Segregated cycling infrastructure*
...
shown to increase cycling uptake [@aldred2019impacts; @goodman2014new;
@marques2015infrastructure], with the separation from motorized
vehicles being key [@winters2011motivators]. Revealed preference of
cyclists shows that they are willing to deviate from the most efficient
routes in order to commute on safer roads [@crane2017longitudinal].
...
relative to the shortest route [@broach2011bicycle; @winters2010far].
...
[@stinson2003commuter]. The lack of well-connected cycling
infrastructure is one of the main obstacles to increasing cycling uptake
[@caulfield2012determining]. While direct and cohesive cycling networks
have been shown to positively impact cycling rates, density^[making an
...
network is also vital [@schoner2014missing].
...
car travel times. Safeguarding against increased car traffic may be counter-productive if the goal is to create a mode-shift, as research has shown that reducing road space for cars leads to less cars on the road, a phenomenon referred to as "traffic evaporation" [@nello2020environmental].
Car usage is not considered by @mauttone2017bicycle,
who develop an optimization framework that aims to minimize the total
user cost of cycling on the network. The aggregate flow^[*flow* is
...
various OD pairs] on links^[*link*
refers to a road segment throughout this research] is obtained by using
...
[@duthie2014optimization]. In this formulation, all origin-destination
(OD) pairs need to be connected by roads that meet the desired LOS. A
limitation of these problem formulations is that they do not explicitly solve for
continuity, which is dealt with using either a constraint specifying
that each link with a bike lane should be connected to at least one
destination [@mesbah2012bilevel], a constraint on maximum deviation
from shortest paths [@duthie2014optimization], or a discontinuity
penalty to prioritize connected road segments [@mauttone2017bicycle].
...
components*. Other academics have taken this approach; @natera2019data
study the existing cycling network in terms of its disconnected
components and introduce two different algorithms to connect these
components by their most critical links and, in doing so, measure the
size of the growth of the largest connected component as a function of
the kilometers of network added. The concept of connected components is
also at the core of the methodology proposed by @olmos2020data. After
...
[@jafino2020transport]. 
<!-- @nahmias2017integrating criticize the -->
...
<!-- area.  -->
The utilitarian approach, historically popular in transport planning, has been criticised for focusing on the bigger picture and failing to account for the distribution of investments on the different communities of the study area [@lucas2016method; @nahmias2017integrating].
@pereira2017distributive emphasize the need for a more
...
disproportionate number of trips within them. @akbarzadeh2018designing
use a modularity maximization approach [@blondel2008fast] on taxi trip
...
connectivity within the communities, not between them. @bao2017planning
...
sub-methodologies that expand on some of the limitations of previous
...
and (3) insufficient consideration of the ethical principles underlying
the analysis. Section \@ref(calculating-potential-cycling-demand)
...
@duthie2014optimization note that relying only on existing cycling activity to prioritise cycling
infrastructure can reinforce existing cycling patterns and ignores potential cycling demand that could be satisfied by a connected network.
To avoid this issue they choose to ignore existing demand completely, and focus on creating a network that connects the
entire study area. 
@olmos2020data opt to calculate potential demand instead; they obtain the distance distribution of cyclists using a smartphone-based bicycle GPS data, and then use a rejection-sampling algorithm on the OD data of the study area to match
the potential demand distribution to the distribution obtained from GPS data.

*OD* data can be obtained from a range of sources,
...
travel volumes between zones [@black1995spatial; @martinez2013new;
@wilson1971family]. In this paper we use open access data from the UK
census [@ONS2011flowdata], which contains aggregate statistics on
...
population of just over 8000 [@ofn2018population].^[ See
<https://wicid.ukdataservice.ac.uk/> for open access to the OD data.]
Figures \@ref(fig:potdemhistograms) and \@ref(fig:desirefacetcycling)
illustrates the proportion of trips cycled by distance and the
...
different scenarios of change (we used the Government Target scenario,
representing a nationwide target of doubling cycling by 2025) as a
function of distance and hilliness [@lovelace2017propensity]. The
...
\noindent where **d** and **s** are the distance and slope respectively
for the OD pair. The authors use square and square-root distance terms
"to capture the non-linear impact of distance on the likelihood of
...
models could be used in different contexts or scenarios of change.
...
```{r desirefacetcycling, fig.cap="Current and Potential Cycling Demand", out.width="60%", fig.pos="H", fig.align="center", echo=FALSE}
knitr::include_graphics("data/Manchester/Plots/desire_facet_cycling.png")
```

It should be noted that the calculations assume cycling in the
traditional sense, i.e. the topography affects the decision to cycle.
Recently there have been various micro-mobility solutions, including
e-bikes, that allow commuters to traverse longer distances and hillier
roads with less effort than traditional bicycles. While these modes
would probably be associated with less geographical impedance, it is
beyond the scope of this work to integrate those modes into the
analysis. This is partially restricted by the lack of data on the
proliferation of these modes, raising the point that perhaps the census
data category of `Bicycle' is too vague, and should be further
disaggregated to distinguish between traditional bicycles and other
...
1.  **Cyclist Preference**: Work done by @dill2013four on examining
    cyclist typologies determined that around 60% of Portland residents
    fit under the *interested but concerned* category. These were people
...
2.  **Low-Traffic Neighborhoods**: The UK Department for Transport is
    ...
    [@departmentgearchange2020]. This includes closing off residential
...
[@mauttone2017bicycle], or by assigning different weights to the road
segments proportional to the investment cost of bringing them to an
acceptable level of stress for cycling [@duthie2014optimization].
Perceived stress for cyclists has been found to be a function of vehicular traffic
volume and speed [@sorton1994bicycle], both of which vary predictably
with road type. @gehrke2020cycling build on this information to use
...
by @sorton1994bicycle and information from Table
\@ref(table:osmroadtypes)), and roads with existing cycling
infrastructure. We believe this to be more practical than the simplistic
approach adopted by @mauttone2017bicycle, as it goes beyond simply
favoring roads with existing cycling infrastructure enabling a hierarchy
of road preference based on perceived stress levels.
...
\noindent A weighted distance $\boldsymbol{d_{w}}$ for each road
segment is calculated as follows ^[The **dodgr** r package
[@padgham2019dodgr] is used to route cycling demand onto the road
...
achieves the same result. For the sake of reproducibility, we stick to
...
\noindent where $\boldsymbol{d_{unw}}$ is the unweighted distance and
$\boldsymbol{W}$ is the weight from Table \ref{table:weightprofiles}.
...
Route Network (PRN) [@department2012guidance]. The PRN has the widest,
...
1.  **Logistical Difficulty**: Changes on these roads need to be agreed
    upon by all affected authorities [@department2012guidance], which
...
2.  **Low Traffic Neighborhoods (LTNs)**: The UK government is aiming to
    ...
    LTNs [@departmentgearchange2020]. This is part of a policy to
...
\@ref(fig:boxplotcircuity). When routing using the *weighted* weighting
...
roads is banned (*weighted_2* profile in Table
...
circuitous cycling networks [@departmentgearchange2020].
...
described in @gehrke2020cycling and @furth2016network. Sensitivity
...
[@departmentgearchange2020], and so having cycling infrastructure
...
[@departmentcycleinfradesign2020]. @goodman2019scenarios show that if
...
cyclists, @dill2013four found that a majority of people who say they
...
A range of algorithms could be used for prioritisation. Because policy
...
[@parkin2018designing]. Community detection offers us a way to
...
Louvain method [@blondel2008fast] is used to separate MSOAs into
...
```{r communitiesmanchester, fig.cap="Communities Based on Potential Cycling Demand (Manchester)", out.width="75%", fig.show='hold', fig.align = "center", echo=FALSE}
...
a result of the routing in Section \@ref(introduction).
...
```{r growthtotal, fig.cap="Comparing Overall (Dashed) and Community Level Person-Km Satisfied (Manchester)", out.width="35%", fig.show='hold', fig.align = "center", echo=FALSE}
knitr::include_graphics(c(
...
```{r growth3MapandBar, fig.cap="Road Segment Priority (left), disaggregated by road type (right) - Egalitarian Growth", out.width="35%", fig.show='hold', fig.align="center", echo=FALSE}
knitr::include_graphics(c(
...
```{r componentsandGCC, fig.cap="Network Characteristics", out.width="30%", fig.show='hold', fig.pos="H", fig.align = "center", echo=FALSE}
knitr::include_graphics(c(
...
local or regional levels. The results, primarily detailed route network
maps based on current travel behaviour derived from OD data, can provide
an evidence-based starting point for introducing a direct, connected,
and low-stress network for any city by leveraging available open data.
...
cities^[The results are easily reproducible for all UK cities, and can
also be reproduced for cities elsewhere given the availability of
commuter data. For instructions on reproducing the results shown in this
...
A core part of the methodology is determining where there is a high
potential for cycling, and using this as a basis for recommendations on
road space reallocation in order to unlock potential cycling demand. The
framework provides a routing engine to study the effects of limiting
access on different road types, finding that reducing cyclist flow on
roads with high through-traffic results in acceptable increases in
commuting distances. As a result, a hierarchy of road preference needs
to be defined to route potential cycling demand. The framework provides
an algorithm to determine where investments in cycling
infrastructure should be prioritized. This algorithm, based on
connectivity and egalitarian principles of resource distribution,
ensured that whatever the level of investment, the resulting cycling
network would improve connectivity of existing cycling infrastructure.
The algorithm was also shown to provide similar city-wide results as well better distribution of investment when compared to one based on utilitarian principles.
...
coarse zones). @iacono2010measuring note that such large travel zones
...
exist in a vacuum. @wardman2007factors developed a mode choice model
...
well as the cycling mode share [@parkin2018designing;
@pucher2010infrastructure]. These cities do not just focus on
...
inconvenient, while soft policies include marketing and awareness compaigns [@gossling2013urban; @pucher2008making]. While these policies are outside the scope of this research, it is important to recognize their key role in
bringing about an increase in levels of cycling.

The methods can support the ongoing shift towards active transport,
which is one of the priority areas of investments worldwide in transport
planning. This necessary shift predates the pandemic and is driven by
ambitions to improve the health and well-being of the citizens
worldwide. The success of such a shift depends on the appeal of the
cycling network for prospective cyclists, together with policies and
interventions deterring the usage of the private car.
