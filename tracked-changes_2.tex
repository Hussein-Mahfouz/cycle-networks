...
%DIF DEL paper_round_1.tex   Thu May  4 21:58:56 2023
%DIF ADD README.tex          Thu May  4 20:26:09 2023
...
But new vehicle technologies can only go so far and \DIFdelbegin \DIFdel{do not }\DIFdelend \DIFaddbegin \DIFadd{fail to }\DIFaddend tackle parallel problems such as congestion, road traffic casualties and physical inactivity (Brand et al., 2020).

In this context, interest and investment in active modes \DIFdelbegin \DIFdel{are growing .
The }\DIFdelend \DIFaddbegin \DIFadd{has grown substantially, especially with the growing realisation that the }\DIFaddend benefits extend beyond congestion and the environment, as active travel can also help alleviate what is referred to as the pandemic of global inactivity; physical inactivity is on the rise and has become the 4th highest cause of death globally (Kohl 3rd et al., 2012).
...
\DIFdelbegin \DIFdel{The funding on its own }\DIFdelend \DIFaddbegin \DIFadd{Funding alone }\DIFaddend is therefore no guarantee of a change in commuting across the country; it must be used to design adequate cycling infrastructure that is based on motivators and deterrents to cycling.
In this paper we propose an approach for planning \DIFdelbegin \DIFdel{a connected cycling network that is meant to
be applicable }\DIFdelend \DIFaddbegin \DIFadd{connected cycling networks that can be generalised }\DIFaddend to any city \DIFaddbegin \DIFadd{with appropriate input data }\DIFaddend (as shown in the Appendix)\DIFdelbegin \DIFdel{, and we use }\DIFdelend \DIFaddbegin \DIFadd{.
We used }\DIFaddend the city of Manchester as a case study \DIFdelbegin \DIFdel{. Greater
Manchester aims }\DIFdelend \DIFaddbegin \DIFadd{due to its ambitions }\DIFaddend to become the first \DIFdelbegin \DIFdel{region to reach }\DIFdelend \DIFaddbegin \DIFadd{UK region with }\DIFaddend a carbon neutral transport network\DIFdelbegin \DIFdel{;
its local authorities have developed }\DIFdelend \DIFaddbegin \DIFadd{, supported by ambitious }\DIFaddend plans for a 1800 mile walking and cycling network (\DIFdelbegin \DIFdel{Bee Network}\DIFdelend \DIFaddbegin \DIFadd{the `Bee Network'}\DIFaddend ) that will be delivered over 10 years (Greater Manchester, 2020).
...
Much research has \DIFdelbegin \DIFdel{been done to understand }\DIFdelend \DIFaddbegin \DIFadd{explored the question of }\DIFaddend what gets people \DIFdelbegin \DIFdel{to cycle}\DIFdelend \DIFaddbegin \DIFadd{cycling}\DIFaddend .
Segregated cycling infrastructure \DIFdelbegin \footnote{\emph{\DIFdel{Segregated cycling infrastructure}}
  %DIFAUXCMD
\DIFdel{refers to road space that is allocated to cyclists only, with physical
  separation to protect cyclists from other modes of transport.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{has been
shown to increase }\DIFdelend \DIFaddbegin \DIFadd{--- road space that is allocated to cyclists only with physical separation to protect cyclists --- is associated with }\DIFaddend cycling uptake (Aldred et al., 2019; Goodman et al., 2014; Marqués et al., 2015)\DIFdelbegin \DIFdel{, with the separation }\DIFdelend \DIFaddbegin \DIFadd{.
Separation }\DIFaddend from motorized vehicles \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{has been found to be }\DIFaddend key (Winters et al., 2011).
...
However, such deviations are only considered if they do not considerably increase route circuitry; \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend studies have found that the probability of choosing a route decreases in proportion to its length relative to the shortest route (Broach et al., 2011; Winters et al., 2010).
...
\DIFaddend Mauttone et al. (2017) developed an optimization framework aiming at minimizing the total user cost of cycling on the network without considering car usage.
...
%DIFDELCMD < %%%
\DIFdelend In this paper, continuity is analysed by looking at the connectivity of the network through the graph-theoretic concept of \emph{connected components}.
\DIFaddbegin 

\DIFaddend Orozco et al. (2020) study the existing cycling network in terms of its disconnected components.
...
After routing the cycling demand onto the network links, they use percolation theory to filter out the links based on the aggregate flow passing through them, varying the flow threshold for filtering to identify the minimum flow at which the whole city is connected by a \DIFdelbegin \DIFdel{giant }\DIFdelend \DIFaddbegin \DIFadd{single }\DIFaddend component.
...
\DIFaddbegin \DIFadd{Other approaches have focused on characteristics of route segments as the basis of analysis to help prioritize investment in cycle infrastructure where it is most needed.
...
\DIFadd{With the exception of Grisé and El-Geneidy (2018), the aforementioned studies do not explore the equity impacts of different network prioritization approaches.
...
\DIFaddend \hypertarget{ethical-underpinnings-and-proposed-approach}{%
...
\DIFaddbegin \hypertarget{method-and-application}{%
...
\DIFaddend This research attempts to propose an egalitarian framework for cycling network design.
...
\DIFaddbegin \DIFadd{As mentioned above, Manchester is used as a case study.
An overview of the steps involved is shown in Figure \ref{fig:methodflowchart}.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \hypertarget{calculating-potential-cycling-demand}{%
%DIFDELCMD < \section{Calculating Potential Cycling Demand}\label{calculating-potential-cycling-demand}}
%DIFDELCMD < %%%
...
\caption{\DIFaddFL{Flowchart of proposed methodology}}\label{fig:methodflowchart}
\end{figure}
...
In this paper we use open access data from the UK census (ONS, 2011), which contains aggregate statistics on number of commuters between administrative zones --- Middle layer Super Output Areas (MSOA) --- by mode of travel \DIFaddbegin \DIFadd{(this is similar to the approach used by Grisé and El-Geneidy (2018))}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \hypertarget{routing}{%
%DIFDELCMD < \section{Routing}\label{routing}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{routing}{%
...
This has been previously done by multiplying all road segments without cycling infrastructure by a fixed impedance factor (Mauttone et al., 2017), or by assigning different weights to the road segments \DIFaddbegin \DIFadd{that are (a) }\DIFaddend proportional to the investment cost of bringing them to an acceptable level of stress for cycling (Duthie and Unnikrishnan, 2014)\DIFaddbegin \DIFadd{, or (b) inversely proportional to perceived level of stress experienced by cyclists (Wysling and Purves, 2022)}\DIFaddend .
...
\DIFaddbegin \DIFadd{It is also different to the weighting profile used by Wysling and Purves (2022) as it adds a larger weight on trunk and primary roads then other roads without cycling infrastructure.
}\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \hypertarget{road-segment-prioritization}{%
%DIFDELCMD < \section{Road Segment Prioritization}\label{road-segment-prioritization}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{road-segment-prioritization}{%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{community-detection}{%
%DIFDELCMD < \subsection{Community Detection}\label{community-detection}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{community-detection}{%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{algorithm-1-utilitarian-expansion}{%
%DIFDELCMD < \subsection{Algorithm 1: Utilitarian Expansion}\label{algorithm-1-utilitarian-expansion}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{algorithm-1-utilitarian-expansion}{%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{algorithm-2-egalitarian-expansion}{%
%DIFDELCMD < \subsection{Algorithm 2: Egalitarian Expansion}\label{algorithm-2-egalitarian-expansion}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{algorithm-2-egalitarian-expansion}{%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{connectivity}{%
%DIFDELCMD < \subsection{Connectivity}\label{connectivity}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{connectivity}{%
...
\leavevmode\vadjust pre{\DIFaddbegin \hypertarget{ref-grise2018if}{}}%DIF > 
\DIFadd{Grisé, E., El-Geneidy, A., 2018. If we build it, who will benefit? A multi-criteria approach for the prioritization of new bicycle lanes in quebec city, canada. Journal of transport and land use 11, 217--235.
}

\leavevmode\vadjust \DIFadd{pre}{\DIFaddend \hypertarget{ref-iacono2010measuring}{}}%
...
\leavevmode\vadjust \DIFadd{pre}{\hypertarget{ref-wysling2022improve}{}}%DIF > 
\DIFadd{Wysling, L., Purves, R.S., 2022. Where to improve cycling infrastructure? Assessing bicycle suitability and bikeability with open data in the city of paris. Transportation research interdisciplinary perspectives 15, 100648.
}\DIFaddend 
